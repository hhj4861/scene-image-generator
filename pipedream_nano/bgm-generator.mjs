...
export default defineComponent({
  ...
  async run({ steps, $ }) {
    ...
    // BGM 관련 정보 추출
    const bgmConfig = scriptOutput.pipeline_data?.bgm || {};
    const mood = bgmConfig.mood || scriptOutput.mood || scriptOutput.input?.target_emotion || "calm";
    const contentStyle = bgmConfig.content_style || scriptOutput.input?.content_style || "pet";
    const musicSuggestion = bgmConfig.music_suggestion || scriptOutput.pipeline_data?.metadata?.music || "";

    // ★ 총 영상 길이 (스크립트 길이 기반 동적 duration)
    const targetDuration = scriptOutput.pipeline_data?.total_duration_seconds ||
