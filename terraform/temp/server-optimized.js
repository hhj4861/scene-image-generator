...
app.post("/render/puppy", async (req, res) => {
    ...
    try {
        ...
        // 타임코드 문자열을 초로 변환하는 함수 (00:03.00 → 3.0)
        ...
        for (const { duration, video, index } of downloadedVideos) {
            ...
            // ★ 타이밍별 자막 배열이 있으면 사용
            if (video.subtitles && Array.isArray(video.subtitles) && video.subtitles.length > 0 && subtitle_enabled) {
                // 첫 번째 자막의 시작 시간을 기준으로 오프셋 계산
                const firstSubStart = parseTimeCode(video.subtitles[0].start_time);

                for (const sub of video.subtitles) {
                    const subStart = parseTimeCode(sub.start_time) - firstSubStart;
                    const subEnd = parseTimeCode(sub.end_time) - firstSubStart;

                    subtitles.push({
                        start: sceneStartTime + subStart + 0.1,
                        end: sceneStartTime + subEnd - 0.1,
                        text: sub.text_ko || "",
                        text_english: sub.text_en || "",
                        speaker: video.speaker || "main",
                        scene_index: sceneNum,
                    });
                }
            } else {
                // ★ 기존 방식: narration 전체를 하나의 자막으로
