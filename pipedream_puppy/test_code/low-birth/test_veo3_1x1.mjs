/**
 * Veo 3 1:1 Aspect Ratio Test
 * 1:1 정사각형 비율로 영상 생성 후 FFmpeg 레이아웃 테스트
 */
...
const FOLDER_NAME = `test_veo3_1x1_${Date.now()}`;
...
console.log("🚀 Veo 3 1:1 (Square) Aspect Ratio Test");
...
// --- Step 1: Generate Video with Veo 3 (1:1) ---
async function generateVeo3Video(imagePath) {
    console.log("🎬 Step 1: Generating video with Veo 3 (1:1 Square)...\n");
...
async function callFFmpegVM(videoUrl) {
    ...
    const payload = {
        ...
        output_path: `${FOLDER_NAME}/final_1x1_test.mp4`,
...
async function main() {
    try {
        ...
        const dimensions = execSync(`ffprobe -v error -select_streams v:0 -show_entries stream=width,height -of csv=s=x:p=0 "${localVideoPath}"`).toString().trim();
        console.log(`  📐 Video dimensions: ${dimensions}\n`);

        // Step 3: Upload to GCS
        const gcsVideoUrl = await uploadToGCS(localVideoPath, `${FOLDER_NAME}/source_1x1.mp4`);
        ...
        console.log(`📊 Source Video (1:1): ${gcsVideoUrl}`);
