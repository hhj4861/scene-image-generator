/**
 * Hedra Character 3 립싱크 테스트 - 빈 프롬프트
 * 프롬프트 없이 순수 오디오 립싱크에 의존
 */
...
async function generateTTS(text, voiceId, outputPath) {
  console.log(`   TTS 생성: "${text}" (voice: ${voiceId.substring(0, 10)}...)`);
...
// Hedra 테스트 - 빈 프롬프트
// =====================
async function testHedraEmptyPrompt() {
  console.log('═══════════════════════════════════════════════════════════');
  console.log('  🎬 Hedra 립싱크 테스트 - 빈 프롬프트');
  console.log('═══════════════════════════════════════════════════════════');
  console.log();
  ...
  try {
    ...
    const puppyScript = '아빠! 나 땅콩이야!';
    const audioPath = path.join(outputFolder, 'audio_empty_prompt_test.mp3');
    ...
    const audioAssetResponse = await axios.post('https://api.hedra.com/web-app/public/assets', {
      name: 'audio_empty_prompt_test.mp3',
      type: 'audio'
    }, {
      headers: { 'x-api-key': CONFIG.HEDRA_API_KEY, 'Content-Type': 'application/json' }
    });
    ...
    audioFormData.append('file', audioBuffer, { filename: 'audio_empty_prompt_test.mp3', contentType: 'audio/mpeg' });
    ...
    // 5. 비디오 생성 요청 - 빈 프롬프트 (text_prompt 생략)
    console.log('🎬 [STEP 5] 비디오 생성 요청 (빈 프롬프트)...');
    ...
    const requestData = {
      ...
      generated_video_inputs: {
        resolution: '720p',
        aspect_ratio: '9:16'
        // text_prompt 생략 - 순수 오디오 립싱크에 의존
      }
    };

    console.log(`   프롬프트: (없음 - 빈 프롬프트)`);
    ...
    const videoFilename = 'hedra_empty_prompt.mp4';
    ...
    console.log();
    console.log('═══════════════════════════════════════════════════════════');
    console.log('  ✅ 빈 프롬프트 테스트 완료!');
    console.log('  📁 결과: hedra_empty_prompt.mp4');
    console.log('═══════════════════════════════════════════════════════════');
